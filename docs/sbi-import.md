...
- `OK` のJSONは、必須の資産値がすべて存在することを検証してからスナップショットと保有情報を保存する。必須値が欠けている場合は取り込みエラーとし、スナップショットを保存しない。明示的なゼロはゼロとして保存する。
- `MAINTENANCE` / `ERROR` のJSONは取得試行の記録としてスナップショットだけ保存する。すべての資産値を `NULL` とし、保有情報は保存しない。
- 保有米国株の前日比は、`OK` でも仕様上取得できないため `NULL` とする。
