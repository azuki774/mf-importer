...
- `schema_version` は文字列の `2026-09-12` のみ受け付けます。未知の文字列版は警告を出してそのファイルだけスキップし、後続ファイルを処理します。欠落、`null`、数値版、壊れたJSON、対応版の内容不備はエラー終了します。
...
- 複数ファイルは辞書順に処理し、未知の文字列版は継続、それ以外の解析・取得・DBエラーが起きた時点で停止する。
...
終了時には対象ファイル数、新規登録数、重複スキップ数、未対応バージョンのスキップ数をログへ出力する。資産金額、数量、銘柄名、入力されたバージョン値などの明細はログへ出力しない。
