...
func TestAPIService_AddRule(t *testing.T) {
	type fields struct {
		Logger *zap.Logger
		Repo   DBRepository
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := a.AddRule(tt.args.ctx, tt.args.req)
			if (err != nil) != tt.wantErr {
				t.Errorf("APIService.AddRule() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("APIService.AddRule() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestAPIService_ResetImportDetails(t *testing.T) {
	type fields struct {
		Logger *zap.Logger
		Repo   DBRepository
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := a.ResetImportDetails(tt.args.ctx, tt.args.id); (err != nil) != tt.wantErr {
				t.Errorf("APIService.ResetImportDetails() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
